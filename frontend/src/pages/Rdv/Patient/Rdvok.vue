            ...
            <table ref="dataTable" class="table table-bordered table-striped">
              ...
              <tbody>
                <tr v-for="(patient, eco) in allPatient" :key="patient.id">
                  ...
                  <td>{{ patient.motif_r }}</td>
                  <td>{{ patient.date_r }}</td>
                  <td>{{ patient.time_r }}</td>
                </tr>
              </tbody>
            </table>
          </div>
...
export default {
  ...
  methods: {
    async fetchPatient() {
      const patient = await fetch("http://127.0.0.1:8000/api/rdvenall/pers/"+localStorage.getItem("current_iduser"), {
        method: "GET",
        headers: {
